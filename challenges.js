...
function reverseUpcaseString(s) {
...
function findHighestPriced(objectArray) {
  let highestIndex = 0;
  let highestPrice = 0;
  for(let i = 0; i < objectArray.length; i++) {
    if(objectArray[i].price > highestPrice) {
      highestPrice = objectArray[i].price;
      highestIndex = i;
    }
  }
...
- The reduceArray function should iterate over each element in the array (first arg).  For each iteration, invoke the callback 
  function (2nd arg), passing to it three arguments: (1) the "accumulator", which is the value returned by the callback during 
  the previous iteration; (2) the  current element; and (3) the index of the current iteration.
